<x-perfect-scrollbar as="nav" aria-label="main" class="flex flex-col flex-1 gap-4 px-3">
    ...
    <x-sidebar.link title="Home" href="{{ route('home.index') }}" :isActive="request()->routeIs('home.index')">
        <x-slot name="icon">
            <x-heroicon-o-home class="flex-shrink-0 w-6 h-6" aria-hidden="true" />
        </x-slot>
    </x-sidebar.link>
    
    <x-sidebar.link title="Film" href="{{ route('film.index') }}" :isActive="request()->routeIs('film.index')">
        <x-slot name="icon">
            <x-heroicon-o-film class="flex-shrink-0 w-6 h-6" aria-hidden="true" />
        </x-slot>
    </x-sidebar.link>

    <x-sidebar.link title="Serial" href="{{ route('serial.index') }}" :isActive="request()->routeIs('serial.index')">
        <x-slot name="icon">
